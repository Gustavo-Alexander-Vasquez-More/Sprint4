...
//PARA LOS CHECKS
export function crearLosCheckbox(categoria) {
  const contCheck=document.createElement("div")
  contCheck.classList.add("form-check")
  contCheck.innerHTML = `<input class="form-check-input" type="checkbox" value="${categoria}" id="${categoria}">
 ...
 export  function ImprimirLosChecks(eventos, $checkbox) {
    let checkSinRepetir = new Set(eventos.filter((elemento) => elemento.category).map((elemento) => elemento.category));
